<?php

$extension_active = true;
$playermode = 'laut';
$extlangpath = 'engine/extensions/' . $playermode . '/lang/' . $language . '.php';
require($extlangpath );

// Debug: Show Path to language file
//echo "<h1>" . $extlangpath . "</h1>";

$extension_logo_html = "<img class='' style='width:min(100%, 128px); height:auto;' alt='laut.fm-Extension by Pangom' src='data:image/svg+xml;base64,'></img>";

$lautExtensionCredit = 'laut.fm-Extension by Pangom';
if (isset($about_extension_link) && $about_extension_link) {
    $extensions_credits .= "<span>" . $extension_logo_html . "</span>";
    $extensions_credits .= "<span class='about-extension-credit-value'><a href='https://laut.fm?from=PanPlay-Extension' target='_blank'>" . $lautExtensionCredit . "</a></span>";
} else {
    $extensions_credits .= "<span>" . $extension_logo_html . "</span>";
    $extensions_credits .= "<span class='about-extension-credit-value'>" . $lautExtensionCredit . "</span>";
}


// Basis-URL für den Stream
$streamBaseUrl = 'https://stream.laut.fm/';

// Streaming-URL erstellen

    $streamUrl = $streamBaseUrl . $lfmstream;


    







        echo "<meta property='og:title' content='🎶" . $lfmstream . " - PanPlay' /> <meta property='og:type' content='audio.livestream' /> <meta property='og:url' content='" . $_SERVER['HTTP_HOST'] . "' /> <meta property='og:description' 
        content='Höre das laut.fm-Radio " . $lfmstream ." in PanPlay! Mit Titelinformationen, Sendeplan, u.v.m.' />";
      


// nolfmw is the global override for all laut.fm feature windows. "y" is the active value;
// "j" is accepted as a legacy alias from older German documentation.
if (isset($_GET['nolfmw']) && ($_GET['nolfmw'] === 'y' || $_GET['nolfmw'] === 'j')) {
    // Wenn "nolfmfeatures" aktiv ist, setzen Sie alle Variablen auf false
    $sendeplan = false;
    $stationinfo = false;
    $playwith = false;
    $trackhistory = false;
    $currentsongmodal= false;
} else {
    // Ansonsten setzen Sie die Variablen entsprechend der anderen URL-Parameter

    // Überprüfen, ob der URL-Parameter "sendeplan" vorhanden ist und ob sein Wert "y" ist
    if (isset($_GET['schedule']) && $_GET['schedule'] === 'y') {
        $sendeplan = true;
    } elseif (isset($_GET['schedule']) && $_GET['schedule'] === 'n') {
        $sendeplan = false;
    } else {
        $sendeplan = true; // Standardwert
    }

    // Überprüfen, ob der URL-Parameter "stationinfo" vorhanden ist und ob sein Wert "y" ist
    if (isset($_GET['stationinfo']) && $_GET['stationinfo'] === 'y') {
        $stationinfo = true;
    } elseif (isset($_GET['stationinfo']) && $_GET['stationinfo'] === 'n') {
        $stationinfo = false;
    } else {
        $stationinfo = true; // Standardwert
    }

      // Überprüfen, ob der lautfm-livemodus (LBN) durch parameter "lbn" aktiviert - also vorhanden ist und ob sein Wert "y" ist
      if (isset($_GET['lbn']) && $_GET['lbn'] === 'y') {
        $lfmlbn = true;
    } elseif (isset($_GET['lbn']) && $_GET['lbn'] === 'n') {
        $lfmlbn = false; // <-- Hier wird $lfmlbn auf false gesetzt
    } else {
        $lfmlbn = false;
    }

    echo '<script type="text/javascript">const isLbnActive = ' . json_encode($lfmlbn) . ';</script>';
    

    // Überprüfen, ob der URL-Parameter "playwith" vorhanden ist und ob sein Wert "y" ist
    if (isset($_GET['playwith']) && $_GET['playwith'] === 'y') {
        $playwith = true;
    } elseif (isset($_GET['playwith']) && $_GET['playwith'] === 'n') {
        $playwith = false;
    } else {
        $playwith = true; // Standardwert
    }

    // Überprüfen, ob der URL-Parameter "trackhistory" vorhanden ist und ob sein Wert "y" ist
    if (isset($_GET['trackhistory']) && $_GET['trackhistory'] === 'y') {
        $trackhistory = true;
    } elseif (isset($_GET['trackhistory']) && $_GET['trackhistory'] === 'n') {
        $trackhistory = false;
    } else {
        $trackhistory = true; // Standardwert
    }

    // Überprüfen, ob der URL-Parameter "trackhistory" vorhanden ist und ob sein Wert "y" ist
    if (isset($_GET['currentsongmodal']) && $_GET['currentsongmodal'] === 'y') {
        $currentsongmodal = true;
    } elseif (isset($_GET['currentsongmodal']) && $_GET['currentsongmodal'] === 'n') {
        $currentsongmodal = false;
    } else {
        $currentsongmodal = true; // Standardwert
    }
}

require('engine/extensions/laut/lautapi.php');
?>
